...
def main():
    ...
    with (APP / 'Contents/Info.plist').open('rb') as f:
        assert plistlib.load(f)['CFBundleShortVersionString'] == '0.1.1'
    ...
    copy(ROOT / 'templates.json', RUNTIME / 'templates.json')
    ...
    dmg = BUILD / 'Scribebot-0.1.1-macOS-arm64.dmg'
    if dmg.exists():
        raise SystemExit('Release DMG already exists; refusing to overwrite it.')
    run('hdiutil', 'create', '-volname', 'Scribebot 0.1.1', '-srcfolder', STAGE,
        '-format', 'UDZO', '-ov', dmg)
